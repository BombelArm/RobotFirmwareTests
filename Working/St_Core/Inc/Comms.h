...
if(connection->status==2)
{
	...
	if(connection->container.what==0)
	{
		if(strcmp(connection->Last_Msg,SYS_INFO)==0)
		{
					...
					char string[16];
					sprintf(string, "%4.3f\n %4.3f\n %4.3f "  ,position_from_encoder[0],position_from_encoder[1],position_from_encoder[2]);
		...
		else if(strcmp(connection->Last_Msg,"TEST_STOP")==0)
				{
		...
		else if(strcmp(connection->Last_Msg,"HOME")==0)
					{
		...
		else if(strcmp(connection->Last_Msg,"INIT_ALL")==0)
						{
							...
							ST_MOT_Init(0,0.25,8000,800);
							ST_MOT_Init(1,0.02,4000,80);
							ST_MOT_Init(2,0.05,5000,400);
							Movement_Prep(0,0);
						    Movement_Prep(1,0);
							Movement_Prep(2,0);
